export interface BusVisionNativePlugin {
  ...
  setPhotoFlashMode(options: { mode: 'auto' | 'on' | 'off' | string }): Promise<{ ok: boolean; mode?: string }>
  setExposureCompensation(options: { index: number }): Promise<{ ok: boolean; index?: number }>
  setZoom(options: { ratio: number }): Promise<{ ok: boolean }>

  exportZip(options: {
    zipName: string
    files: Array<{ name: string; text?: string; sourcePath?: string }>
    share?: boolean
  }): Promise<{ ok: boolean; uri: string; path?: string; count: number; sizeBytes: number; shared?: boolean }>
}
