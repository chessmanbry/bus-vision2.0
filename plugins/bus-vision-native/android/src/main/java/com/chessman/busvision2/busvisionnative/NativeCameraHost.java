...
public class NativeCameraHost {
  ...
  private volatile String photoFlashMode = "off";
  private volatile int exposureCompensationIndex = 0;
  ...
  private void bindUseCases(CameraStartCallback cb) {
    ...
    try {
      ...
      imageCapture = new ImageCapture.Builder()
        ...
        .build();
      applyPhotoFlashMode(photoFlashMode);
      ...
      applyExposureCompensation(exposureCompensationIndex);
  ...
  private void applyPhotoFlashMode(String mode) {
    if (imageCapture == null) return;
    String m = mode == null ? "off" : mode.toLowerCase();
    if ("on".equals(m)) imageCapture.setFlashMode(ImageCapture.FLASH_MODE_ON);
    else if ("auto".equals(m)) imageCapture.setFlashMode(ImageCapture.FLASH_MODE_AUTO);
    else imageCapture.setFlashMode(ImageCapture.FLASH_MODE_OFF);
  }

  public boolean setPhotoFlashMode(String mode) {
    String m = mode == null ? "off" : mode.toLowerCase();
    if (!"auto".equals(m) && !"on".equals(m) && !"off".equals(m)) m = "off";
    photoFlashMode = m;
    applyPhotoFlashMode(photoFlashMode);
    return true;
  }

  private boolean applyExposureCompensation(int targetIndex) {
    if (camera == null) return false;
    try {
      androidx.camera.core.ExposureState state = camera.getCameraInfo().getExposureState();
      if (state == null || !state.isExposureCompensationSupported()) return false;
      android.util.Range<Integer> range = state.getExposureCompensationRange();
      int safe = Math.max(range.getLower(), Math.min(range.getUpper(), targetIndex));
      exposureCompensationIndex = safe;
      camera.getCameraControl().setExposureCompensationIndex(safe);
      return true;
    } catch (Throwable t) {
      Log.w(TAG, "set exposure compensation failed", t);
      return false;
    }
  }

  public boolean setExposureCompensation(int index) {
    exposureCompensationIndex = index;
    return applyExposureCompensation(index);
  }

  public int getExposureCompensationIndex() { return exposureCompensationIndex; }
  public String getPhotoFlashMode() { return photoFlashMode; }
