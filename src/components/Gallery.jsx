...
async function fileToBase64Payload(file) {
  return {
    base64: await blobToBase64Payload(file),
    sizeBytes: file.size || 0,
    mimeType: file.type || 'video/mp4',
  };
}

async function webPathToBase64(webPath) {
  const response = await fetch(webPath);
  const blob = await response.blob();
  return { base64: await blobToBase64Payload(blob), sizeBytes: blob.size, mimeType: blob.type || 'image/jpeg' };
}

function imageExtFromMime(mimeType, fallback = 'jpg') {
...
const EXPORT_PREFIX_KEY = 'busVisionExportPrefix';

function sanitizeExportPrefix(value) {
  const cleaned = String(value || '')
    .trim()
    .replace(/[^a-zA-Z0-9_-]+/g, '_')
    .replace(/^_+|_+$/g, '')
    .slice(0, 48);
  return cleaned || 'busvision';
}
...
async function writeAndShareZip(zipName, files) {
  // Android 實機優先走原生串流 ZIP：避免 WebView 把照片/影片/WAV 全部讀成 base64 後爆記憶體。
  // 0.8.9.3 修正：Gallery 必須明確 import BusVisionNative，否則按「匯出訓練」會直接 ReferenceError。
  if (Capacitor.isNativePlatform?.()) {
    if (!BusVisionNative?.exportZip) throw new Error('Android 原生匯出模組沒有載入，請重新 npm run build + npx cap sync android 後安裝。');
    const result = await BusVisionNative.exportZip({ zipName, files, share: true });
    // 0.8.9.4：Android 改由原生 plugin 在 ZIP 建立後直接開分享面板，
    // 避免 Capacitor Share 對 content:// ZIP uri 在部分 WebView / Android 版本不彈窗。
    // 若原生端未回報已開啟分享，才走舊的 Share fallback。
    if (!result.shared) {
      try {
        await Share.share({ title: zipName, text: 'Bus Vision dataset export', url: result.uri, dialogTitle: '匯出資料集 ZIP' });
      } catch (shareErr) {
        const msg = String(shareErr?.message || shareErr || '');
        if (!/cancel|canceled|cancelled/i.test(msg)) console.warn('Share fallback failed after ZIP created:', shareErr);
      }
    }
    return { zipPath: result.path || zipName, count: result.count || files.length, sizeBytes: result.sizeBytes || 0, shared: !!result.shared };
  }

  // Web / fallback：只在沒有原生方法時使用舊的 JS ZIP。
  await Filesystem.mkdir({ path: EXPORT_ROOT, directory: Directory.Cache, recursive: true }).catch(() => {});
  const normalizedFiles = [];
  for (const f of files) {
    if (f.sourcePath) normalizedFiles.push({ name: f.name, bytes: await readDataFileBytes(f.sourcePath) });
    else normalizedFiles.push(f);
  }
  const zipBytes = createStoredZip(normalizedFiles);
...
async function buildYoloZip(items, exportPrefix = 'busvision') {
  ...
  for (const item of imageItems) {
    ...
    try {
      ...
      const stem = `${exportPrefix}_${item.id}`;
      const label = yoloLabelLine(item, classIndexMap);
      files.push({ name: `images/train/${stem}.${ext}`, sourcePath });
  ...
  return writeAndShareZip(`${exportPrefix}_yolo_${Date.now()}.zip`, files);
}

async function buildOcrZip(items, exportPrefix = 'busvision') {
  ...
  for (const item of items.filter((it) => isPhotoLikeItem(it) || it.snapshotPath)) {
    ...
    try {
      const ext = extFromPath(sourcePath, 'jpg');
      const stem = `${exportPrefix}_ocr_${item.id}`;
      files.push({ name: `images/${stem}.${ext}`, sourcePath });
      const ann = bestAnnotation(item);
  ...
  return writeAndShareZip(`${exportPrefix}_ocr_${Date.now()}.zip`, files);
}

async function buildAudioZip(items, exportPrefix = 'busvision') {
  ...
  for (const item of audioItems) {
    try {
      ...
      const audioName = `${exportPrefix}_audio_${item.id}.${ext}`;
      files.push({ name: `audio/${audioName}`, sourcePath: item.audioPath });
  ...
  return writeAndShareZip(`${exportPrefix}_audio_${Date.now()}.zip`, files);
}

async function buildFullZip(items, exportPrefix = 'busvision') {
  ...
  for (const item of items) {
    for (const key of ['path', 'snapshotPath', 'audioPath']) {
      ...
      try {
        ...
        files.push({ name: `raw/${exportPrefix}_${key}_${item.id}.${ext}`, sourcePath: item[key] });
      } catch (_) {}
    }
  }
  ...
  return writeAndShareZip(`${exportPrefix}_full_${Date.now()}.zip`, files);
}
...
export default function Gallery() {
  ...
  const [exportPrefix, setExportPrefix] = useState(() => localStorage.getItem(EXPORT_PREFIX_KEY) || 'busvision');
  ...
  const [pendingRectNorm, setPendingRectNorm] = useState(null);
  ...
  const importExistingPhotos = async () => {
    try {
      ...
      for (const photo of photos) {
        ...
        if (!webPath) continue;
        const payload = await webPathToBase64(webPath);
        const ts = safeNowId() + created.length;
        const ext = imageExtFromMime(photo.format ? `image/${photo.format}` : payload.mimeType, 'jpg');
        const filename = `IMPORT_${ts}.${ext}`;
        const path = `${FOLDER_NAME}/${filename}`;
        await Filesystem.writeFile({ directory: Directory.Data, path, data: payload.base64 });
        created.push({
          id: ts,
          type: 'photo',
          source: 'photo_library_import',
          imported: true,
          createdAt: new Date().toISOString(),
          path,
          filename,
          mimeType: payload.mimeType || (ext === 'png' ? 'image/png' : 'image/jpeg'),
          sizeBytes: payload.sizeBytes,
          captureProfile: 'import_existing_photo',
          taskMode: 'import_existing_photo',
          tags: {
            lighting: 'unknown',
            distance: 'unknown',
            scene: 'imported',
          ...
          annotations: [],
          audioAnnotations: [],
          metadataNote: '從手機相簿匯入，可在已標注/待補標流程中繼續標注。',
        });
      }
      if (!created.length) {
        showToast('匯入失敗：沒有可讀取的照片');
        return;
      }
      const next = [...created, ...current].sort((a, b) => (b.id || 0) - (a.id || 0));
      await writeMetadata(next);
      setItems(next);
      setActiveTab('todo');
      showToast(`已匯入 ${created.length} 張照片`);
      speak(`已匯入 ${created.length} 張照片`);
    } catch (e) {
      const msg = String(e?.message || e || '未知錯誤');
      if (/cancel/i.test(msg)) return;
      showToast(`匯入照片失敗：${msg}`, 2400);
    }
  };


  const importExistingVideosFromFiles = async (fileList) => {
    try {
      const files = Array.from(fileList || []).filter((f) => String(f?.type || '').startsWith('video/') || /\.(mp4|mov|m4v|webm|mkv)$/i.test(f?.name || ''));
      if (!files.length) {
        showToast('沒有選取影片');
        return;
      }
      ...
      for (const file of files) {
        ...
        const ts = safeNowId() + created.length;
        const ext = videoExtFromMime(payload.mimeType, (file.name?.split('.').pop() || 'mp4').toLowerCase());
        const filename = `IMPORT_VIDEO_${ts}.${ext}`;
        const path = `${FOLDER_NAME}/${filename}`;
        await Filesystem.writeFile({ directory: Directory.Data, path, data: payload.base64 });
        created.push({
          id: ts,
          type: 'video',
          source: 'video_library_import',
          imported: true,
          createdAt: new Date().toISOString(),
          path,
          filename,
          originalName: file.name || filename,
          mimeType: payload.mimeType || `video/${ext}`,
          sizeBytes: payload.sizeBytes,
          captureProfile: 'import_existing_video',
          taskMode: 'import_existing_video',
          tags: {
            lighting: 'unknown',
            distance: 'unknown',
            scene: 'imported_video',
            soundStage: '',
            quality: 'unreviewed',
          },
          annotation: {
          ...
          annotations: [],
          audioAnnotations: [],
          metadataNote: '從手機相簿/檔案匯入的影片，可在資料庫中預覽、截圖標注，並建立聲音區段標籤。',
        });
      }
      if (!created.length) {
        showToast('匯入失敗：沒有可讀取的影片');
        return;
      }
      const next = [...created, ...current].sort((a, b) => (b.id || 0) - (a.id || 0));
      await writeMetadata(next);
      setItems(next);
      setActiveTab('todo');
      showToast(`已匯入 ${created.length} 支影片`);
      speak(`已匯入 ${created.length} 支影片`);
    } catch (e) {
      const msg = String(e?.message || e || '未知錯誤');
      if (/cancel/i.test(msg)) return;
      showToast(`匯入影片失敗：${msg}`, 2600);
    } finally {
      if (videoImportInputRef.current) videoImportInputRef.current.value = '';
    }
  };

  const importExistingVideos = () => {
  ...
  const runDatasetExport = async (kind) => {
    ...
    try {
      ...
      const safePrefix = sanitizeExportPrefix(exportPrefix);
      setExportPrefix(safePrefix);
      localStorage.setItem(EXPORT_PREFIX_KEY, safePrefix);
      let result;
      if (kind === 'yolo') result = await buildYoloZip(fresh, safePrefix);
      else if (kind === 'ocr') result = await buildOcrZip(fresh, safePrefix);
      else if (kind === 'audio') result = await buildAudioZip(fresh, safePrefix);
      else result = await buildFullZip(fresh, safePrefix);
      setExportSummary(`已建立 ZIP 並開啟分享：${result.count} 個檔案，${Math.round(result.sizeBytes / 1024)} KB`);
      showToast('已建立 ZIP，請在分享面板選擇儲存位置');
    } catch (e) {
  ...
  const saveAnnotation = async () => {
    ...
    try {
      ...
      const ann = {
        id: editingAnnotationId || safeNowId(),
        createdAt: editingAnnotationId ? (old.find((a) => a.id === editingAnnotationId)?.createdAt || now) : now,
        updatedAt: now,
      ...
      const nextAnnotations = editingAnnotationId
        ? old.map((a) => a.id === editingAnnotationId ? ann : a)
        : [ann, ...old];
      data[idx] = {
        ...data[idx],
      ...
      await writeMetadata(data);
      showToast(editingAnnotationId ? '已更新標注' : '已儲存標注');
      speak('已儲存標注');
      setAnnotateOpen(false);
      setEditingAnnotationId(null);
      setPendingRectNorm(null);
  ...
  const styles = useMemo(
    () => ({
      ...
      annotateToolbar: { display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 10, alignItems: 'center', marginBottom: 8 },
      annotateHint: { color: '#cfcfcf', fontSize: 12, lineHeight: 1.35 },
  ...
  const annotatedRecords = useMemo(() => {
    return items.flatMap((it) => {
      const imagePath = isPhotoItem(it) ? it.path : it.snapshotPath;
      const anns = annotationsOf(it).map((ann, idx) => ({
        recordType: 'image-annotation',
        key: `${it.id}-${ann.id || idx}`,
        item: it,
        ann,
        imagePath,
        title: ann.busNo || ann?.ocr?.routeText || boxClassLabel(ann.boxClass),
        subtitle: `${boxClassLabel(ann.boxClass)}｜${ann.createdAt || ''}`,
      }));
      return anns;
    }).filter((r) => r.imagePath);
  }, [items]);

  const openAnnotatedRecord = async (item, ann) => {
    try {
      setActiveItem(item);
      const sourcePath = isPhotoItem(item) ? item.path : item.snapshotPath;
      if (!sourcePath) {
        showToast('這筆標注沒有可顯示的圖片截圖，請重新截圖標注');
        return;
      }
      const src = await getPlayableSrcFromDataPath(sourcePath);
      setSnapshotSrc(src);
      setSuggestText(ann?.engineSuggestion?.text || '已載入既有標注');
      setDetBoxes([]);
      setEditingAnnotationId(ann?.id || null);
      setBusNo(ann?.busNo || ann?.ocr?.routeText || '');
      setBoxClass(ann?.boxClass || item?.annotation?.preset || 'bus');
      setVisibilityTag(ann?.quality?.visibility || 'clear');
      setOcclusionTag(ann?.quality?.occlusion || 'none');
      setBlurTag(ann?.quality?.blur || 'sharp');
      setAnnoSoundStage(ann?.soundStage || item?.tags?.soundStage || item?.telemetry?.soundDistanceStage || '');
      setPendingRectNorm(ann?.manualRect || null);
      setManualRectView(null);
      setDrawMode(false);
      setPreviewOpen(false);
      setAnnotateOpen(true);
      showToast('已載入既有標注，可修改後儲存');
    } catch (e) {
      showToast(`開啟已標注圖片失敗：${String(e?.message || e)}`, 2000);
    }
  };
  ...
  const filteredItems = items.filter((it) => {
    if (activeTab === 'annotated') return false;
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.sub}>{loading ? '讀取中…' : `${viewName(activeTab)}：${activeTab === 'annotated' ? annotatedRecords.length : filteredItems.length} / 共 ${items.length} 筆資料｜可從相簿上傳既有相片/影片`}</div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.05fr 1.05fr 1fr', gap: 8, marginBottom: 12 }}>
        <input
          ...
          multiple
          style={{ display: 'none' }}
          onChange={(e) => importExistingVideosFromFiles(e.target.files)}
        />
        <button
          onClick={reload}
          style={{
        ...
        <input
          ...
          onBlur={(e) => {
            const safe = sanitizeExportPrefix(e.target.value);
            setExportPrefix(safe);
            localStorage.setItem(EXPORT_PREFIX_KEY, safe);
          }}
          placeholder="匯出前綴"
          style={{
            height: 42,
      ...
      <div style={styles.tabBar}>
        {[
          ['all', `全部 ${items.length}`],
          ['annotated', `已標注 ${annotatedRecords.length}`],
      ...
      {activeTab === 'annotated' ? (
        <div style={styles.grid}>
          {annotatedRecords.map((rec) => (
            <div key={rec.key} style={styles.card} onClick={() => openAnnotatedRecord(rec.item, rec.ann)}>
              <div style={styles.row}>
                <div>
                  <div style={styles.title}>🖼️ {rec.title || '已標注圖片'}</div>
                  <div style={styles.meta}>{rec.subtitle}<br />來源：{rec.item.filename || `Bus_${rec.item.id}`}｜點開可看到上次框線並修改</div>
                </div>
                <div style={{ display: 'grid', gap: 6, justifyItems: 'end' }}><div style={{ ...styles.qualityPill, background: '#1689ff' }}>可修改</div><div style={styles.pill}>開啟</div></div>
              </div>
            </div>
          ))}
          {!annotatedRecords.length ? <div style={styles.historyBox}>尚未有可顯示的已標注圖片。影片需要先截圖並儲存標注後，才會出現在這裡。</div> : null}
        </div>
      ) : (
        <div style={styles.grid}>
        ...
        <div style={styles.sheetOverlay} onClick={() => setExportOpen(false)}>
          <div style={styles.sheet} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.sheetBody}>
              ...
              <label style={{ display: 'grid', gap: 6, marginBottom: 12, color: '#ddd', fontSize: 13, fontWeight: 900 }}>
                匯出檔名前綴
                <input
                  value={exportPrefix}
                  onChange={(e) => setExportPrefix(e.target.value)}
                  ...
                  placeholder="例如 taipei_bus、route_307"
                  style={{ height: 42, borderRadius: 12, border: '1px solid rgba(255,255,255,0.16)', background: 'rgba(0,0,0,0.26)', color: '#fff', padding: '0 12px', outline: 'none', fontWeight: 800 }}
                />
                <span style={{ color: '#999', fontSize: 12, fontWeight: 500 }}>只保留英文、數字、底線、減號；ZIP 與資料集內部檔名都會套用。</span>
              </label>
        ...
        <div style={styles.overlay} onClick={() => setAnnotateOpen(false)}>
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div style={styles.modalHead}>
              <div style={styles.modalTitle}>{editingAnnotationId ? '修改標注' : '開始標注'}</div>
            ...
            <div style={styles.modalBody}>
              ...
              <div style={styles.section}>
                <div style={styles.annotateToolbar}>
                  <button style={drawMode ? styles.chipOn : styles.chipOff} onClick={() => setDrawMode(!drawMode)}>{drawMode ? '框選模式：開' : '框選模式：關'}</button>
                  <span style={styles.annotateHint}>{drawMode ? '現在可拉框/拖曳框；若要滑動頁面請先關閉。' : '目前可順暢滑動頁面；需要改框時開啟框選模式。'}</span>
                </div>
                <div
                  ref={snapBoxRef}
                  style={{ ...styles.snapStage, touchAction: drawMode ? 'none' : 'pan-y' }}
                ...
                >
                  {snapshotSrc ? (
                    <img
                      ...
                      onLoad={() => {
                        setDetBoxes([]);
                        if (pendingRectNorm && snapBoxRef.current) {
                          requestAnimationFrame(() => {
                            const box = snapBoxRef.current?.getBoundingClientRect();
                            if (box) {
                              setManualRectView({
                                x: Number(pendingRectNorm.x || 0) * box.width,
                                y: Number(pendingRectNorm.y || 0) * box.height,
                                w: Number(pendingRectNorm.w || 0) * box.width,
                                h: Number(pendingRectNorm.h || 0) * box.height,
                              });
                            }
                          });
                        } else {
