...
const PHOTO_FLASH_SEQUENCE = ['auto', 'on', 'off'];
const EXPOSURE_MODES = {
  auto: { label: '自動曝光', ev: 0, hint: '一般自動曝光' },
  night: { label: '夜間', ev: -1, hint: '夜間低光，稍微壓暗避免過曝' },
  anti_headlight: { label: '抗車燈', ev: -2, hint: '壓低高亮車燈與反光' },
  ocr_priority: { label: 'OCR優先', ev: -3, hint: '優先保留 LED/路線牌文字細節' }
};
const TASK_MODES = [
...
export default function Recorder() {
  ...
  const [photoFlashMode, setPhotoFlashMode] = useState('auto');
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      if (IS_ANDROID_NATIVE) {
        ...
        try {
          await BusVisionNative.setPhotoFlashMode({ mode: photoFlashMode });
          await BusVisionNative.setExposureCompensation({ index: exposureEv });
          await BusVisionNative.setTorch({ enabled: torchEnabled });
        } catch (_) {}
        setCameraReady(true);
        setStatus('原生相機已就緒');
        return;
  ...
  const buildBaseMetadata = async (kind) => {
    ...
    return {
      ...
      type: kind,
      createdAt: nowIso(),
      collectorVersion: 'bus-vision2.0.0.8.7.1-night-exposure-buildfix',
  ...
  const toggleTorch = async () => {
    ...
    if (!IS_ANDROID_NATIVE) return;
    try {
      await BusVisionNative.setTorch({ enabled: next });
      setStatus(next ? '補光燈：長亮開啟' : '補光燈：關閉');
    } catch (err) {
      setStatus(`補光燈不可用：${err?.message || err}`);
    }
  };

  const applyPhotoFlashMode = async (mode) => {
    setPhotoFlashMode(mode);
    if (!IS_ANDROID_NATIVE) return;
    try {
      await BusVisionNative.setPhotoFlashMode({ mode });
      setStatus(mode === 'auto' ? '拍照閃光：自動' : mode === 'on' ? '拍照閃光：拍照瞬閃' : '拍照閃光：關閉');
    } catch (err) {
      setStatus(`拍照閃光不可用：${err?.message || err}`);
    }
  };

  const cyclePhotoFlash = () => {
    const currentIndex = PHOTO_FLASH_SEQUENCE.indexOf(photoFlashMode);
    const next = PHOTO_FLASH_SEQUENCE[(currentIndex + 1) % PHOTO_FLASH_SEQUENCE.length];
    applyPhotoFlashMode(next);
  };

  const applyExposureMode = async (mode) => {
    const cfg = EXPOSURE_MODES[mode] || EXPOSURE_MODES.auto;
    setExposureMode(mode);
    setExposureEv(cfg.ev);
    // OCR 優先與抗車燈時，關閉長亮補光避免反光干擾 LED/車牌。
    if ((mode === 'ocr_priority' || mode === 'anti_headlight') && torchEnabled) {
      setTorchEnabled(false);
      if (IS_ANDROID_NATIVE) BusVisionNative.setTorch({ enabled: false }).catch(() => {});
    }
    if (IS_ANDROID_NATIVE) {
      try { await BusVisionNative.setExposureCompensation({ index: cfg.ev }); } catch (_) {}
    }
    setStatus(`曝光模式：${cfg.label} / EV ${cfg.ev}`);
  };

  const cycleExposureMode = () => {
    const seq = ['auto', 'night', 'anti_headlight', 'ocr_priority'];
    const next = seq[(seq.indexOf(exposureMode) + 1) % seq.length];
    applyExposureMode(next);
  };
  ...
  return (
    <div style={styles.shell} onClick={handlePreviewTap}>
      ...
      <div style={styles.topIconCluster} onClick={(e) => e.stopPropagation()}>
        <IconButton title="日夜間" label="日夜" active={lightingMode !== 'auto'} onClick={cycleLighting}>{lightingIcon(lightingMode)}</IconButton>
        <IconButton title="補光燈長亮" label="補光" active={torchEnabled} onClick={toggleTorch}>{torchEnabled ? '💡' : '🔦'}</IconButton>
        <IconButton title="拍照瞬閃" label="瞬閃" active={photoFlashMode !== 'off'} onClick={cyclePhotoFlash}>{photoFlashIcon(photoFlashMode)}</IconButton>
        <IconButton title="夜間曝光控制" label="曝光" active={exposureMode !== 'auto'} onClick={cycleExposureMode}>{exposureIcon(exposureMode)}</IconButton>
      ...
      {recording && <div style={styles.recordingBadge}>● REC</div>}
      ...
      <div style={{ ...styles.drawer, transform: drawerOpen ? 'translateY(0)' : 'translateY(100%)' }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={styles.drawerContent}>
          ...
          <SectionTitle>夜間曝光 / 閃光</SectionTitle>
          <RowScroll>
            {Object.keys(EXPOSURE_MODES).map((m) => <Chip key={m} active={exposureMode === m} onClick={() => applyExposureMode(m)}>{exposureIcon(m)} {EXPOSURE_MODES[m].label}</Chip>)}
          </RowScroll>
          <div style={styles.helpText}>目前 EV：{exposureEv}。抗車燈與 OCR 優先會壓低曝光，保留 LED 路線牌與高亮車燈細節。</div>
          <RowScroll>
            <Chip active={torchEnabled} onClick={toggleTorch}>{torchEnabled ? '💡 補光長亮 ON' : '🔦 補光長亮 OFF'}</Chip>
            {PHOTO_FLASH_SEQUENCE.map((m) => <Chip key={m} active={photoFlashMode === m} onClick={() => applyPhotoFlashMode(m)}>⚡ 拍照閃光 {photoFlashLabel(m)}</Chip>)}
          </RowScroll>
...
function photoFlashIcon(mode) {
  if (mode === 'on') return '⚡';
  if (mode === 'off') return '⚡︎';
  return 'A⚡';
}
function photoFlashLabel(mode) {
  if (mode === 'on') return '開';
  if (mode === 'off') return '關';
  return '自動';
}
function exposureIcon(mode) {
  if (mode === 'ocr_priority') return '🔤';
  if (mode === 'anti_headlight') return '🚘';
  if (mode === 'night') return '🌙';
  return 'EV';
}
...
const styles = {
  ...
  topIconCluster: { position: 'absolute', top: 12, right: 8, zIndex: 40, display: 'flex', alignItems: 'center', gap: 5 },
  recordingBadge: { position: 'absolute', top: 18, left: 14, zIndex: 41, padding: '7px 10px', borderRadius: 999, background: 'rgba(200,0,0,0.76)', border: '1px solid rgba(255,255,255,0.24)', color: '#fff', fontWeight: 1000, fontSize: 12, letterSpacing: 0.8 },
  iconBtn: { width: 43, height: 44, borderRadius: 17, border: '1px solid rgba(255,255,255,0.26)', background: 'linear-gradient(180deg, rgba(255,255,255,0.18), rgba(0,0,0,0.46))', color: '#fff', fontWeight: 1000, fontSize: 15, display: 'grid', placeItems: 'center', gap: 0, padding: '3px 2px', boxShadow: '0 8px 22px rgba(0,0,0,0.32)', backdropFilter: 'blur(14px)' },
  ...
  focusHud: { position: 'absolute', right: 12, top: 'calc(env(safe-area-inset-top, 0px) + 74px)', zIndex: 31, width: 168, maxWidth: '46vw', borderRadius: 16, background: 'rgba(0,0,0,0.46)', border, backdropFilter: 'blur(14px)', overflow: 'hidden' },
  ...
  zoomPanel: { position: 'absolute', left: 18, right: 18, bottom: 126, zIndex: 36, borderRadius: 18, background: 'rgba(0,0,0,0.46)', border, backdropFilter: 'blur(16px)', boxShadow: '0 12px 30px rgba(0,0,0,0.30)', overflow: 'hidden' },
  ...
  bottomControls: { position: 'absolute', bottom: 28, left: 0, right: 0, zIndex: 35, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 22, pointerEvents: 'auto' },
  ...
  drawerHeader: { height: 48, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', borderBottom: '1px solid rgba(255,255,255,0.10)' },
  drawerCloseBtn: { width: 36, height: 36, borderRadius: 12, border: '1px solid rgba(255,255,255,0.16)', background: 'rgba(255,255,255,0.08)', color: '#fff', fontSize: 20, fontWeight: 1000 },
  drawerContent: { padding: '0 14px 18px', overflowY: 'auto', maxHeight: 'calc(72vh - 48px)' },
